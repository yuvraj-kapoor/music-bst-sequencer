...
void reverse_insert_notes(BST_Node *root, BST_Node **new_root, int min_bar, int max_bar)
{
    int new_bar;
    double new_index;
    BST_Node *new_node;
    ...
    reverse_insert_notes(root->left, new_root, min_bar, max_bar);

    new_bar = min_bar + max_bar - root->bar;
    new_index = 1.0 - root->index;

    new_node = newBST_Node(root->freq, new_bar, new_index);
    *new_root = BST_insert(*new_root, new_node);

    reverse_insert_notes(root->right, new_root, min_bar, max_bar);
}
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    min_bar = min_node->bar;
    max_bar = max_node->bar;

    new_root = NULL;
    reverse_insert_notes(root, &new_root, min_bar, max_bar);
 ...
 * ******************************************************************************************/
int find_frequency_index(double freq)
{
    int i;
...
void harmonize_helper(BST_Node *node, BST_Node **root, int semitones, double time_shift)
{
    BST_Node *left_original;
    BST_Node *right_original;
    int i;
    int new_i;
    double new_freq;
    double new_index;
    ...
    i = find_frequency_index(node->freq);
    if (i != -1)
    {
        new_i = i + semitones;
        new_index = node->index + time_shift;

        if (new_i >= 0 && new_i < 100 && new_index >= 0 && new_index < 1)
        {
            new_freq = note_freq[new_i];
            new_node = newBST_Node(new_freq, node->bar, new_index);
            *root = BST_insert(*root, new_node);
        }
    }

    harmonize_helper(left_original, root, semitones, time_shift);
    harmonize_helper(right_original, root, semitones, time_shift);
}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    harmonize_helper(root, &root, semitones, time_shift);
    return root;
